# Graph Report - QuantAI  (2026-05-09)

## Corpus Check
- 125 files · ~208,479 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3006 nodes · 5151 edges · 262 communities detected
- Extraction: 74% EXTRACTED · 26% INFERRED · 0% AMBIGUOUS · INFERRED: 1358 edges (avg confidence: 0.76)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 139|Community 139]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]

## God Nodes (most connected - your core abstractions)
1. `Client` - 98 edges
2. `BrokerBase` - 84 edges
3. `check_risk()` - 42 edges
4. `IBKRBroker` - 40 edges
5. `_trade()` - 35 edges
6. `_account()` - 34 edges
7. `post()` - 31 edges
8. `_beta()` - 29 edges
9. `reconcile_ghost_positions()` - 29 edges
10. `run()` - 28 edges

## Surprising Connections (you probably didn't know these)
- `run()` --calls--> `main()`  [INFERRED]
  v2/shared-data/scripts/autonomous_execution.py → discord-bot/bot.py
- `main()` --calls--> `get_positions()`  [INFERRED]
  v2/shared-data/scripts/close_intc_wings.py → discord-bot/alpaca_client.py
- `sync()` --calls--> `on_ready()`  [INFERRED]
  v2/shared-data/scripts/sheets_sync.py → discord-bot/bot.py
- `place_mleg_order()` --calls--> `get_positions()`  [INFERRED]
  v2/shared-data/scripts/autonomous_execution.py → discord-bot/alpaca_client.py
- `run()` --calls--> `run_claude_code()`  [INFERRED]
  v2/shared-data/scripts/autonomous_execution.py → scripts/cto_listener.py

## Hyperedges (group relationships)
- **Three-Agent Self-Learning System (Alpha/Beta/Gamma)** — agent_alpha_identity_who_i_am, agent_beta_identity_who_i_am, agent_gamma_identity_who_i_am [INFERRED 0.90]
- **Cross-Agent Shared Skills (greeks/execution/exits)** — greeks_management_theta, execution_quality_mleg_combo, position_exit_timing_alpha_rules, agent_alpha_identity_skills_loaded, agent_beta_identity_skills_loaded, agent_gamma_identity_skills_loaded [INFERRED 0.85]
- **IBKR Migration → Beta Plan Replan Chain** — adr_004_ibkr_migration, beta_plan_v1_context, beta_plan_v2_context, claude_md_ib_gateway [INFERRED 0.90]

## Communities

### Community 0 - "Community 0"
Cohesion: 0.01
Nodes (155): _direction(), Broken Wing Butterfly — XSP. Asymmetric profit if market stays in range.  Regime, select_strikes(), AlpacaBroker, BrokerBase, _build_occ(), _build_occ_from_contract(), IBKRBroker (+147 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (140): _call_haiku(), Call Haiku with a tight timeout. Returns text or None on failure., Call Haiku with a tight timeout. Returns text or None on failure., Run capability diagnosis for a closed trade. Never raises., Load AGENT_*_IDENTITY.md content. Returns empty string if missing., Find trades that were OPEN overlapping with this trade's lifetime., Read journal tail to check for any status==OPEN trades. Read-only.      Returns, Post to Discord; return message id on success, None otherwise.      Uses the sam (+132 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (124): build_context(), _build_summary(), _cache_path(), _get_agent1_params(), _get_agent2_params(), main(), context_builder.py — Pre-Trade Context Score (0–100) ===========================, Event calendar score — 15 points max. (+116 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (68): _build_context(), _concurrent_positions(), diagnose(), _load_agent_identity(), main(), _parse_json_response(), Run capability diagnosis for a closed trade. Never raises., Load AGENT_*_IDENTITY.md content. Returns empty string if missing. (+60 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (119): _close_position(), _estimate_condor_current_cost(), guard_check(), load_params(), log_trade(), make_embed(), monitor_positions(), post_discord() (+111 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (83): get_broker(), Return the active broker singleton.      Reads BROKER_TYPE env var (alpaca|ibkr), build_closing_legs(), build_occ(), check_beta_exit(), check_exit_threshold(), check_gamma_exit(), compute_trade_pnl() (+75 more)

### Community 6 - "Community 6"
Cohesion: 0.04
Nodes (54): _is_in_restart_window(), IB Gateway restarts at 23:45 ET. Refuse to connect 23:30-01:30 ET.      Window w, Structured logging helper for QuantAI Python scripts.  Adds a SQLiteHandler to t, Add a SQLiteHandler to the root logger. Idempotent: calling twice does nothing., Logging handler that inserts WARNING+ records into pyapp_inbox table.      The c, Create the inbox table if the DB exists but the table doesn't.         Does NOT, setup(), SQLiteHandler (+46 more)

### Community 7 - "Community 7"
Cohesion: 0.04
Nodes (76): build_occ_symbol(), check_guards(), check_no_overlapping_positions(), count_open_agent_trades(), execute_bear_call_spread(), execute_bull_put_spread(), execute_diagonal_spread(), execute_generic_spread() (+68 more)

### Community 8 - "Community 8"
Cohesion: 0.05
Nodes (49): check_risk(), _hours_since(), _is_beta(), _monday_iso(), open_beta_positions(), Beta-specific risk engine. Implements spec § 5.  Independent from Alpha — counts, Returns (allowed, reason, possibly-modified trade).      The trade may have its, _today_iso() (+41 more)

### Community 9 - "Community 9"
Cohesion: 0.04
Nodes (49): BaseModel, check_bull_put(), check_cooldown(), check_covered_call(), check_daily_loss(), check_earnings_blackout(), check_halted(), check_iron_condor() (+41 more)

### Community 10 - "Community 10"
Cohesion: 0.05
Nodes (83): get_snapshot(), Get a full snapshot: latest trade, quote, bar., ChannelRouter, cmd_analyze(), cmd_brief(), cmd_emergency_stop(), cmd_guard_check(), cmd_resume() (+75 more)

### Community 11 - "Community 11"
Cohesion: 0.04
Nodes (54): build_prompt(), main(), post_discord(), process_task(), cto_listener.py — Dockerized CTO Task Runner ===================================, Read the queue file. Returns the task dict if status=pending,     else None. Mat, Update the queue file's status field., Build the Claude Code prompt, matching the bash script's format. (+46 more)

### Community 12 - "Community 12"
Cohesion: 0.04
Nodes (76): main(), parse_args(), cancel_all_orders(), cancel_order(), close_all_positions(), close_position(), _generate_hash(), get_account() (+68 more)

### Community 13 - "Community 13"
Cohesion: 0.04
Nodes (78): chat_with_claude(), ChatAgent, cmd_decide(), cmd_remember(), handle_cto_scan(), handle_cto_task(), handle_lessons(), handle_remember() (+70 more)

### Community 14 - "Community 14"
Cohesion: 0.05
Nodes (38): _make_diagnosis_file(), _make_trade(), Integration tests: weekly_synthesis aggregates trade data + posts to Discord., Sonnet failure after retry → fallback text, still writes report., TestSynthesizeWithData, _write_trades(), Regression test: weekly_synthesis.py must hold an exclusive run-lock.  Before 20, A second concurrent attempt must NOT block — non-blocking flock. (+30 more)

### Community 15 - "Community 15"
Cohesion: 0.04
Nodes (38): aggregate_status(), check_clawroute_18790(), check_collector_staleness(), check_cron_freshness(), check_dashboard_html_size(), check_disk(), check_graphify(), check_ibkr_port() (+30 more)

### Community 16 - "Community 16"
Cohesion: 0.05
Nodes (32): _journal_write(), _load_strategy(), main(), _next_beta_id(), _post_discord(), _simulate_slippage(), _write_dashboard_state(), age_of() (+24 more)

### Community 17 - "Community 17"
Cohesion: 0.04
Nodes (70): Agent Alpha Core Principles, Diagonal Spreads (Primary Strategy), Agent Alpha — What I Do NOT Do, Why Agents Did Not Trade, discord.py, py_vollib, FastAPI (guard-engine dep), alpaca-py (+62 more)

### Community 18 - "Community 18"
Cohesion: 0.05
Nodes (62): days_since_earnings(), days_to_earnings(), _fetch_window(), Finnhub earnings-calendar helper for Agent Gamma.  Indices (XSP/SPX/NDX/RUT) and, Trading days until the next earnings announcement, or None if unknown., Trading days since the last earnings announcement, or None if unknown., _consume_pending_file(), _journal_write() (+54 more)

### Community 19 - "Community 19"
Cohesion: 0.06
Nodes (26): get_ibkr_probe_state(), probe_ibkr_port(), Return True if IBKR Gateway port 4002 is accepting connections., Read persistent probe failure state (survives process restarts)., Persist probe state; return updated state dict., update_ibkr_probe_state(), _make_pipeline_beat(), _make_socket_mock() (+18 more)

### Community 20 - "Community 20"
Cohesion: 0.08
Nodes (24): Return list of result dicts for open/submitted orders.          If *client_order, Return list of result dicts for open/submitted orders.          If *client_order, _FakeBrokerBase, _make_broker(), _make_trade_mock(), pm(), Unit tests: Phase 5 partial-fill safeguard.  Three areas under test:   1. IBKRBr, Tests for the 'order_submitted=True then exception' recovery path.      We test (+16 more)

### Community 21 - "Community 21"
Cohesion: 0.07
Nodes (18): _entry_ids(), _load_catalog_doc(), _load_entries(), Unit tests: error-catalog.json and referenced runbook files.  Validates the cata, Guard against high-severity labels on known-noise patterns.      These are patte, Pattern must match the exact string logged by _broker_ibkr.py., Load the full envelope dict., Return the errors list (what lib_errors.load_catalog returns). (+10 more)

### Community 22 - "Community 22"
Cohesion: 0.05
Nodes (48): Alpaca API Gotchas (mleg qty, no position_intent), Cron Schedule, Journal Agent Alpha/Beta Trade Tracking, Journal Agent Trade Closing Protocol, Journal Agent Trade Logging Protocol, Journal Agent Operating Manual, Journal Agent Core Belief (journal as most valuable asset), Journal Agent Soul (trading diary identity) (+40 more)

### Community 23 - "Community 23"
Cohesion: 0.07
Nodes (28): _group_capability_items(), _group_parameter_items(), _latest_weekly_report(), _load_capability_requests(), _load_parameter_suggestions(), _load_tracker(), main(), Generate a deterministic ID so resolutions persist across collector runs. (+20 more)

### Community 24 - "Community 24"
Cohesion: 0.08
Nodes (36): backtest_agent1_params(), backtest_agent2_params(), get_trade_pairs(), load_journal(), backtester.py — Param Change Validator ========================================, Re-simulate a historical Agent 1 trade with different params.     Returns what t, Backtest Agent 1 param change against last N days of journal data.     Returns v, Backtest Agent 2 param change. Uses 60 days (longer because CC trades are weekly (+28 more)

### Community 25 - "Community 25"
Cohesion: 0.05
Nodes (42): Agent Alpha Skills Loaded, Beta Event Strangles, Beta 12-Regime Classification System, Agent Beta Skills Loaded, Connors RSI Pullback Strategy, Agent Gamma Skills Loaded, Stuck OPEN trades A008/A009/A010 finding, Beta Plan v2 Phases (10 phases) (+34 more)

### Community 26 - "Community 26"
Cohesion: 0.1
Nodes (11): is_command_safe(), is_path_allowed(), Returns (allowed, reason)., Position-aware shell command validation. Returns (safe, reason).      Order matt, Run ALL safety checks. Even if LLM tagged fix_class=safe_auto, this gate     sti, validate_proposal(), Unit tests for sentinel_agent.py code-enforced safety rails.  CRITICAL: this is, Even if LLM tags safe_auto, trading-path files MUST be rejected. (+3 more)

### Community 27 - "Community 27"
Cohesion: 0.08
Nodes (31): analyze_overall_correlation(), analyze_per_signal_correlation(), build_correlation_embed(), _build_summary(), load_context_scores(), load_trade_outcomes(), load_weights(), match_scores_to_outcomes() (+23 more)

### Community 28 - "Community 28"
Cohesion: 0.06
Nodes (11): Regression tests: ensure the manual trading workflow stays REMOVED.  The manual, Defensive guard preserved as REQUIRES_SHARES (renamed from MANUAL_ONLY)., self_evolution.py and fetch_sofi.py must be retired (renamed), not present as li, eod_summary.py must show only Alpha + Beta + Gamma. No Amit, no SOFI, no manual_, No Manual Trades tab; agent filter widened to all agent_* prefixes., No collar scan function; no 'collars' mode branch., TestAutonomousExecutionGuard, TestEodSummaryClean (+3 more)

### Community 29 - "Community 29"
Cohesion: 0.09
Nodes (17): assert_learning_state_schema(), assert_open_item_schema(), journal_path(), _make_trade(), populated_journal(), Shared fixtures and assertion helpers for the self-learning test suite., Write two sample trades to the journal and return the path., Create a sandboxed QUANTAI_RUNTIME_ROOT and wire all _paths constants.      Yiel (+9 more)

### Community 30 - "Community 30"
Cohesion: 0.11
Nodes (29): analyze_bull_put_spread(), analyze_covered_call(), analyze_iron_condor(), analyze_option(), classify_moneyness(), compute_greeks(), compute_iv(), dte_to_years() (+21 more)

### Community 31 - "Community 31"
Cohesion: 0.15
Nodes (26): action_restart_service(), action_retry(), classify_lines(), handle_known(), handle_unknown(), line_matches_entry(), load_catalog(), load_dedup() (+18 more)

### Community 32 - "Community 32"
Cohesion: 0.12
Nodes (18): post_to_channel(), Single Discord posting surface for v2 cron scripts. Bot-token only.  Use post_to, line_signature(), load_catalog(), log(), main(), match_catalog(), now_et() (+10 more)

### Community 33 - "Community 33"
Cohesion: 0.12
Nodes (24): compute_adx_14(), compute_bb_width_percentile(), compute_ema_slope(), compute_iv_rank_252d(), compute_spx_chain_metrics(), SPX-derived intelligence helpers for Agent Beta.  market_intelligence.py imports, ATM straddle, implied move %, ATM bid-ask spread, 25-delta put/call skew.     Fa, ADX(14) per spec § 3B. Pandas Series in. Float out, or None on failure. (+16 more)

### Community 34 - "Community 34"
Cohesion: 0.12
Nodes (6): _get_crontab(), Unit tests: identity file structural validation (Gap 11) + cron entries (Gap 12), Return the root crontab as a string, or None if unavailable.      The pipeline c, Verify each expected cron entry is present in the root crontab., test_cron_entry_exists(), TestIdentityFiles

### Community 35 - "Community 35"
Cohesion: 0.14
Nodes (17): ADR-001: Adopt Graphify for Knowledge Graphs, ADR-004: Migrate Alpaca to IBKR, Agent Alpha — Who I Am, Agent Beta — Who I Am, Agent Gamma — Who I Am, Infra Agent Operating Manual, Beta Plan v1 Context (SUPERSEDED), Beta v1 INSTRUMENT_MAP (SPY/VXX proxies) (+9 more)

### Community 36 - "Community 36"
Cohesion: 0.25
Nodes (14): discover_tickers(), get_avg_volume(), get_earnings(), get_iv_rank(), get_price(), get_technicals(), Pull liquid, optionable tickers from multiple sources., Poor man's covered call/put — diagonal spread.     Sell near-term option, buy fu (+6 more)

### Community 37 - "Community 37"
Cohesion: 0.36
Nodes (10): bold_header(), color_rows_by_status(), ensure_sheets(), get_sheet_id_by_name(), load_trades(), Green for CLOSED wins, red for CLOSED losses, yellow for OPEN., setup(), sync() (+2 more)

### Community 38 - "Community 38"
Cohesion: 0.33
Nodes (8): build_event_moves(), _classify(), _fetch_finnhub_events(), main(), Walk back `lookback_days`, classify events, fetch SPX moves, keep last `window`, Query Finnhub economic calendar for [start, end]. US-only events., Absolute % SPX move on `target` vs prior trading day. None if missing., _spx_abs_move()

### Community 39 - "Community 39"
Cohesion: 0.29
Nodes (1): Unit tests for _paths.py — env-var override and constant structure.

### Community 40 - "Community 40"
Cohesion: 0.29
Nodes (2): Smoke test: run system_monitor.py against the live VPS.  Verifies the report fil, TestLiveHealthReport

### Community 41 - "Community 41"
Cohesion: 0.29
Nodes (7): Architecture Overview (KARNA, QuantAI Pipeline, LiteLLM, Dashboard), KARNA + QuantAI Project Overview, Rationale: Never Modify openclaw.service (Type=simple), README Architecture Diagram, Docker Deploy and Test, VPS Infrastructure (Hetzner CX31), sync_workspaces.sh

### Community 42 - "Community 42"
Cohesion: 0.5
Nodes (4): build_case(), Pure-Python Bull/Bear case templates for debate_chamber.py. Eliminates 2 Haiku L, Return a 4-5 bullet markdown case FOR (bull) or AGAINST (bear) the trade.      D, _vix_key()

### Community 43 - "Community 43"
Cohesion: 0.4
Nodes (5): ADR-002: ClawRoute as Single LLM Ingress, Auto-Heal Layer, Legacy error_detector.py Notice, Auto-Heal Runbook Purpose, Centralized Logger Architecture

### Community 44 - "Community 44"
Cohesion: 0.67
Nodes (2): check(), check_keys()

### Community 45 - "Community 45"
Cohesion: 0.5
Nodes (4): Design Principles (LLMs for judgment, cost-conscious, data sovereignty), Rationale: Design Principles Trade-offs, API Keys Configuration, Data Sources (yfinance, Finnhub, Alpha Vantage, Alpaca)

### Community 46 - "Community 46"
Cohesion: 0.67
Nodes (3): Discord Server and Bot Setup, Four Agents (Orchestrator, Research, Infra, Journal), Discord Channels (#chat, #research, #infra, #journal)

### Community 47 - "Community 47"
Cohesion: 0.67
Nodes (3): Orchestrator Soul: What Winning Looks Like, Success Metrics (win rate targets, live capital), Pre-Live Checklist

### Community 48 - "Community 48"
Cohesion: 0.67
Nodes (3): SOFI 5 Trigger Actions, SOFI Collar (200 paper shares), SOFI Collar Strategy

### Community 49 - "Community 49"
Cohesion: 0.67
Nodes (3): Alpaca API Gotchas, Bug1: mleg qty missing, Bug2: position_intent rejected

### Community 50 - "Community 50"
Cohesion: 0.67
Nodes (3): check_sector_correlation function, Sector Mapping Table, Sector Correlation Hard/Soft Rules

### Community 51 - "Community 51"
Cohesion: 0.67
Nodes (3): Phase 4: orchestrator vs v2 double-trading audit, Phase 5: trader-guards usage audit, Known Issues (git divergence, legacy containers)

### Community 52 - "Community 52"
Cohesion: 1.0
Nodes (1): Central path constants for all self-learning scripts.  All scripts that referenc

### Community 53 - "Community 53"
Cohesion: 1.0
Nodes (2): Script Inventory, system_test.py (43-check health test)

### Community 54 - "Community 54"
Cohesion: 1.0
Nodes (2): Key File Paths, Security Rules (never expose .env, credentials)

### Community 55 - "Community 55"
Cohesion: 1.0
Nodes (2): Research Agent Operating Manual, Research Agent Core Beliefs (edge via research)

### Community 56 - "Community 56"
Cohesion: 1.0
Nodes (2): Cron Schedule (UTC), Auto-Heal Schedule (4x/day)

### Community 57 - "Community 57"
Cohesion: 1.0
Nodes (2): Infra Health Check Procedure, Scripts Inventory

### Community 58 - "Community 58"
Cohesion: 1.0
Nodes (2): Agent Beta Core Principles, Agent Beta — What I Do NOT Do

### Community 59 - "Community 59"
Cohesion: 1.0
Nodes (2): Agent Gamma Core Principles, Agent Gamma — What I Do NOT Do

### Community 60 - "Community 60"
Cohesion: 1.0
Nodes (2): Credit Spreads (Secondary), IV Skew (Put vs Call)

### Community 61 - "Community 61"
Cohesion: 1.0
Nodes (2): Severity-to-Discord Behavior, Logger Sources Ingested (systemd/docker/textlog/syslog/pyapp)

### Community 63 - "Community 63"
Cohesion: 1.0
Nodes (1): autonomous_execution.py does `os.makedirs("/root/quantai-v2/...")`         at im

### Community 70 - "Community 70"
Cohesion: 1.0
Nodes (1): Log a decision. Categories: rule_change, strategy, architecture, bug_fix, config

### Community 71 - "Community 71"
Cohesion: 1.0
Nodes (1): # TODO: Phase 2 — trigger Research Agent via orchestrator

### Community 72 - "Community 72"
Cohesion: 1.0
Nodes (1): # TODO: Phase 2 — trigger Analysis Agent

### Community 73 - "Community 73"
Cohesion: 1.0
Nodes (1): # TODO: Write halt flag to shared state / call orchestrator

### Community 74 - "Community 74"
Cohesion: 1.0
Nodes (1): # TODO: Forward to execution agent

### Community 75 - "Community 75"
Cohesion: 1.0
Nodes (1): Equity options trade 09:30–16:00 ET on weekdays.

### Community 76 - "Community 76"
Cohesion: 1.0
Nodes (1): Fetch open positions through the active broker.      Returns {occ_symbol: positi

### Community 77 - "Community 77"
Cohesion: 1.0
Nodes (1): Resolve a leg's OCC symbol. Beta journal entries store the broker-     correct O

### Community 78 - "Community 78"
Cohesion: 1.0
Nodes (1): Sum unrealized_pl across all legs present in Alpaca.     Returns (total_pnl, leg

### Community 79 - "Community 79"
Cohesion: 1.0
Nodes (1): Build reversed legs for a closing mleg order.     Skips legs with no active Alpa

### Community 80 - "Community 80"
Cohesion: 1.0
Nodes (1): Place a close order through the active broker. 1 leg → plain order;     2-4 legs

### Community 81 - "Community 81"
Cohesion: 1.0
Nodes (1): Days to the soonest-expiring leg.

### Community 82 - "Community 82"
Cohesion: 1.0
Nodes (1): Load cached market_intelligence.macro. Refreshes once per minute.

### Community 83 - "Community 83"
Cohesion: 1.0
Nodes (1): Read current Beta regime from dashboard state. Cached per minute.

### Community 84 - "Community 84"
Cohesion: 1.0
Nodes (1): Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur

### Community 85 - "Community 85"
Cohesion: 1.0
Nodes (1): Sum live signed deltas across legs. Returns None if any leg can't be quoted.

### Community 86 - "Community 86"
Cohesion: 1.0
Nodes (1): Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r

### Community 87 - "Community 87"
Cohesion: 1.0
Nodes (1): Agent Gamma exit checks (Connors RSI pullback).      Triggers (in order):

### Community 88 - "Community 88"
Cohesion: 1.0
Nodes (1): Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r

### Community 89 - "Community 89"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr

### Community 90 - "Community 90"
Cohesion: 1.0
Nodes (1): Return True if we have not alerted for *symbol* within the cooldown window.

### Community 91 - "Community 91"
Cohesion: 1.0
Nodes (1): Compare broker option positions against journal entries.      THREE failure mode

### Community 92 - "Community 92"
Cohesion: 1.0
Nodes (1): Return the root crontab as a string, or None if unavailable.      The pipeline c

### Community 93 - "Community 93"
Cohesion: 1.0
Nodes (1): Verify each expected cron entry is present in the root crontab.

### Community 94 - "Community 94"
Cohesion: 1.0
Nodes (1): Alert message must tell operator what to check.

### Community 95 - "Community 95"
Cohesion: 1.0
Nodes (1): Fail-open: corrupt cooldown file → allow alert.

### Community 96 - "Community 96"
Cohesion: 1.0
Nodes (1): Post a message to Discord via bot token. Falls back to webhook if set.

### Community 97 - "Community 97"
Cohesion: 1.0
Nodes (1): Post a trade execution alert to #alerts channel.

### Community 98 - "Community 98"
Cohesion: 1.0
Nodes (1): OCC format: SYMBOL + YYMMDD + C/P + 8-digit strike (strike*1000, zero-padded)

### Community 99 - "Community 99"
Cohesion: 1.0
Nodes (1): Convert '7DTE', '0DTE' etc to YYYY-MM-DD. Never returns today.

### Community 100 - "Community 100"
Cohesion: 1.0
Nodes (1): Query the active broker for available strikes on a given expiry.     Returns sor

### Community 101 - "Community 101"
Cohesion: 1.0
Nodes (1): Find the strike closest to target from available list.

### Community 102 - "Community 102"
Cohesion: 1.0
Nodes (1): Find two real strikes for a spread from Alpaca's chain.     Returns (short_strik

### Community 103 - "Community 103"
Cohesion: 1.0
Nodes (1): Place a multi-leg options order as a single mleg order.     This avoids the 'unc

### Community 104 - "Community 104"
Cohesion: 1.0
Nodes (1): Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin

### Community 105 - "Community 105"
Cohesion: 1.0
Nodes (1): Bear call spread: SELL lower call + BUY higher call.

### Community 106 - "Community 106"
Cohesion: 1.0
Nodes (1): Iron condor: bull put spread + bear call spread as single 4-leg mleg order.

### Community 107 - "Community 107"
Cohesion: 1.0
Nodes (1): Fallback for any other spread — detect put/call from legs and build mleg.

### Community 108 - "Community 108"
Cohesion: 1.0
Nodes (1): Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n

### Community 109 - "Community 109"
Cohesion: 1.0
Nodes (1): Route to correct executor based on strategy. Returns fill or None.

### Community 110 - "Community 110"
Cohesion: 1.0
Nodes (1): Place a close order through the active broker. 1 leg → plain order;     2-4 legs

### Community 111 - "Community 111"
Cohesion: 1.0
Nodes (1): Days to the soonest-expiring leg.

### Community 112 - "Community 112"
Cohesion: 1.0
Nodes (1): Load cached market_intelligence.macro. Refreshes once per minute.

### Community 113 - "Community 113"
Cohesion: 1.0
Nodes (1): Read current Beta regime from dashboard state. Cached per minute.

### Community 114 - "Community 114"
Cohesion: 1.0
Nodes (1): Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur

### Community 115 - "Community 115"
Cohesion: 1.0
Nodes (1): Sum live signed deltas across legs. Returns None if any leg can't be quoted.

### Community 116 - "Community 116"
Cohesion: 1.0
Nodes (1): Return the latest cached daily indicators for `symbol` from     gamma_indicator_

### Community 117 - "Community 117"
Cohesion: 1.0
Nodes (1): Agent Gamma exit checks (Connors RSI pullback).      Triggers (in order):

### Community 118 - "Community 118"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr

### Community 119 - "Community 119"
Cohesion: 1.0
Nodes (1): Return True if we have not alerted for *symbol* within the cooldown window.

### Community 120 - "Community 120"
Cohesion: 1.0
Nodes (1): Compare broker option positions against journal open entries.      Fires a 🔴 Dis

### Community 121 - "Community 121"
Cohesion: 1.0
Nodes (1): Stocks under $30 suitable for Amit's manual collar strategy.

### Community 122 - "Community 122"
Cohesion: 1.0
Nodes (1): Pattern must match the exact string logged by _broker_ibkr.py.

### Community 123 - "Community 123"
Cohesion: 1.0
Nodes (1): Runbook must include the fix command so it's actionable from Discord.

### Community 124 - "Community 124"
Cohesion: 1.0
Nodes (1): Auto-action must be 'none' — gateway restart requires human confirmation.

### Community 125 - "Community 125"
Cohesion: 1.0
Nodes (1): Runbook should reference the IBC config fix so it's self-contained.

### Community 126 - "Community 126"
Cohesion: 1.0
Nodes (1): Runbook must include a way to verify the fix worked.

### Community 127 - "Community 127"
Cohesion: 1.0
Nodes (1): If everything placed cleanly, delete the pending file. If anything     failed, r

### Community 128 - "Community 128"
Cohesion: 1.0
Nodes (1): Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin

### Community 129 - "Community 129"
Cohesion: 1.0
Nodes (1): Bear call spread: SELL lower call + BUY higher call.

### Community 130 - "Community 130"
Cohesion: 1.0
Nodes (1): Iron condor: bull put spread + bear call spread as single 4-leg mleg order.

### Community 131 - "Community 131"
Cohesion: 1.0
Nodes (1): Fallback for any other spread — detect put/call from legs and build mleg.

### Community 132 - "Community 132"
Cohesion: 1.0
Nodes (1): Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n

### Community 133 - "Community 133"
Cohesion: 1.0
Nodes (1): Route to correct executor based on strategy. Returns fill or None.

### Community 134 - "Community 134"
Cohesion: 1.0
Nodes (1): Read persistent probe failure state (survives process restarts).

### Community 135 - "Community 135"
Cohesion: 1.0
Nodes (1): Persist probe state; return updated state dict.

### Community 136 - "Community 136"
Cohesion: 1.0
Nodes (1): NYSE equity options hours: 09:30 → 16:00 ET, Mon-Fri.

### Community 137 - "Community 137"
Cohesion: 1.0
Nodes (1): Decide whether to print the pipeline-beat status line this cron tick.     During

### Community 138 - "Community 138"
Cohesion: 1.0
Nodes (1): Persist the last-print timestamp + status. Failure is non-fatal.

### Community 139 - "Community 139"
Cohesion: 1.0
Nodes (1): Return age in seconds of an ISO8601 timestamp. 0 if missing/unparseable.

### Community 140 - "Community 140"
Cohesion: 1.0
Nodes (1): Map RSI(10) reading to a 1-10 conviction score.      Connors threshold is 30. De

### Community 141 - "Community 141"
Cohesion: 1.0
Nodes (1): Map Beta strike-selection comfort to a 1-10 conviction score.      Heuristic: ho

### Community 142 - "Community 142"
Cohesion: 1.0
Nodes (1): Map debate-chamber judge score (0-100) to a 1-10 conviction score.

### Community 143 - "Community 143"
Cohesion: 1.0
Nodes (1): If everything placed cleanly, delete the pending file. If anything     failed, r

### Community 144 - "Community 144"
Cohesion: 1.0
Nodes (1): Merge `fields` into the entry whose id == trade_id. Atomic rewrite.      Returns

### Community 145 - "Community 145"
Cohesion: 1.0
Nodes (1): Return the entry matching trade_id, or None if absent.

### Community 146 - "Community 146"
Cohesion: 1.0
Nodes (1): Get current price — tries fast_info first (no 404s), falls back to info.

### Community 147 - "Community 147"
Cohesion: 1.0
Nodes (1): Get average volume without triggering fundamentals 404s.

### Community 148 - "Community 148"
Cohesion: 1.0
Nodes (1): Pull liquid, optionable tickers from multiple sources.

### Community 149 - "Community 149"
Cohesion: 1.0
Nodes (1): Poor man's covered call/put — diagonal spread.     Sell near-term option, buy fu

### Community 150 - "Community 150"
Cohesion: 1.0
Nodes (1): Iron condors — range-bound, collect from both sides.

### Community 151 - "Community 151"
Cohesion: 1.0
Nodes (1): Stocks under $30 suitable for Amit's manual collar strategy.

### Community 152 - "Community 152"
Cohesion: 1.0
Nodes (1): If everything placed cleanly, delete the pending file. If anything     failed, r

### Community 153 - "Community 153"
Cohesion: 1.0
Nodes (1): Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin

### Community 154 - "Community 154"
Cohesion: 1.0
Nodes (1): Bear call spread: SELL lower call + BUY higher call.

### Community 155 - "Community 155"
Cohesion: 1.0
Nodes (1): Iron condor: bull put spread + bear call spread as single 4-leg mleg order.

### Community 156 - "Community 156"
Cohesion: 1.0
Nodes (1): Fallback for any other spread — detect put/call from legs and build mleg.

### Community 157 - "Community 157"
Cohesion: 1.0
Nodes (1): Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n

### Community 158 - "Community 158"
Cohesion: 1.0
Nodes (1): Route to correct executor based on strategy. Returns fill or None.

### Community 159 - "Community 159"
Cohesion: 1.0
Nodes (1): Write UTC timestamp to pipeline beat file so heartbeat_monitor can check livenes

### Community 160 - "Community 160"
Cohesion: 1.0
Nodes (1): Read current Beta regime from dashboard state. Cached per minute.

### Community 161 - "Community 161"
Cohesion: 1.0
Nodes (1): Read current Beta regime from dashboard state. Cached per minute.

### Community 162 - "Community 162"
Cohesion: 1.0
Nodes (1): Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur

### Community 163 - "Community 163"
Cohesion: 1.0
Nodes (1): Sum live signed deltas across legs. Returns None if any leg can't be quoted.

### Community 164 - "Community 164"
Cohesion: 1.0
Nodes (1): Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r

### Community 165 - "Community 165"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr

### Community 166 - "Community 166"
Cohesion: 1.0
Nodes (1): Bear call spread: SELL lower call + BUY higher call.

### Community 167 - "Community 167"
Cohesion: 1.0
Nodes (1): Iron condor: bull put spread + bear call spread as single 4-leg mleg order.

### Community 168 - "Community 168"
Cohesion: 1.0
Nodes (1): Fallback for any other spread — detect put/call from legs and build mleg.

### Community 169 - "Community 169"
Cohesion: 1.0
Nodes (1): Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n

### Community 170 - "Community 170"
Cohesion: 1.0
Nodes (1): Route to correct executor based on strategy. Returns fill or None.

### Community 171 - "Community 171"
Cohesion: 1.0
Nodes (1): Build reversed legs for a closing mleg order.     Skips legs with no active Alpa

### Community 172 - "Community 172"
Cohesion: 1.0
Nodes (1): Place a close order through the active broker. 1 leg → plain order;     2-4 legs

### Community 173 - "Community 173"
Cohesion: 1.0
Nodes (1): Days to the soonest-expiring leg.

### Community 174 - "Community 174"
Cohesion: 1.0
Nodes (1): Load cached market_intelligence.macro. Refreshes once per minute.

### Community 175 - "Community 175"
Cohesion: 1.0
Nodes (1): Read current Beta regime from dashboard state. Cached per minute.

### Community 176 - "Community 176"
Cohesion: 1.0
Nodes (1): Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur

### Community 177 - "Community 177"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason).      For Beta trades (exit_rules present):

### Community 178 - "Community 178"
Cohesion: 1.0
Nodes (1): Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r

### Community 179 - "Community 179"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr

### Community 180 - "Community 180"
Cohesion: 1.0
Nodes (1): Build the leg dict in the broker.place_mleg_order shape.

### Community 181 - "Community 181"
Cohesion: 1.0
Nodes (1): Post a trade execution alert to #alerts channel.

### Community 182 - "Community 182"
Cohesion: 1.0
Nodes (1): OCC format: SYMBOL + YYMMDD + C/P + 8-digit strike (strike*1000, zero-padded)

### Community 183 - "Community 183"
Cohesion: 1.0
Nodes (1): Convert '7DTE', '0DTE' etc to YYYY-MM-DD. Never returns today.

### Community 184 - "Community 184"
Cohesion: 1.0
Nodes (1): Query the active broker for available strikes on a given expiry.     Returns sor

### Community 185 - "Community 185"
Cohesion: 1.0
Nodes (1): Find the strike closest to target from available list.

### Community 186 - "Community 186"
Cohesion: 1.0
Nodes (1): Find two real strikes for a spread from Alpaca's chain.     Returns (short_strik

### Community 187 - "Community 187"
Cohesion: 1.0
Nodes (1): Place a multi-leg options order as a single mleg order.     This avoids the 'unc

### Community 188 - "Community 188"
Cohesion: 1.0
Nodes (1): Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin

### Community 189 - "Community 189"
Cohesion: 1.0
Nodes (1): Bear call spread: SELL lower call + BUY higher call.

### Community 190 - "Community 190"
Cohesion: 1.0
Nodes (1): Iron condor: bull put spread + bear call spread as single 4-leg mleg order.

### Community 191 - "Community 191"
Cohesion: 1.0
Nodes (1): Route to correct executor based on strategy. Returns fill or None.

### Community 192 - "Community 192"
Cohesion: 1.0
Nodes (1): Write UTC timestamp to pipeline beat file so heartbeat_monitor can check livenes

### Community 193 - "Community 193"
Cohesion: 1.0
Nodes (1): Build OCC option symbol from journal leg fields.     e.g. XOM 2026-06-18 call 15

### Community 194 - "Community 194"
Cohesion: 1.0
Nodes (1): Equity options trade 09:30–16:00 ET on weekdays.

### Community 195 - "Community 195"
Cohesion: 1.0
Nodes (1): Merge updates into matching journal entries, rewrite atomically.     updates: {t

### Community 196 - "Community 196"
Cohesion: 1.0
Nodes (1): Fetch open positions through the active broker.      Returns {occ_symbol: positi

### Community 197 - "Community 197"
Cohesion: 1.0
Nodes (1): Sum unrealized_pl across all legs present in Alpaca.     Returns (total_pnl, leg

### Community 198 - "Community 198"
Cohesion: 1.0
Nodes (1): Build reversed legs for a closing mleg order.     Skips legs with no active Alpa

### Community 199 - "Community 199"
Cohesion: 1.0
Nodes (1): Place a close order through the active broker. 1 leg → plain order;     2-4 legs

### Community 200 - "Community 200"
Cohesion: 1.0
Nodes (1): Check all four exit rules in priority order.     Returns (should_close: bool, ex

### Community 201 - "Community 201"
Cohesion: 1.0
Nodes (1): Load cached market_intelligence.macro. Refreshes once per minute.

### Community 202 - "Community 202"
Cohesion: 1.0
Nodes (1): Sum live signed deltas across legs. Returns None if any leg can't be quoted.

### Community 203 - "Community 203"
Cohesion: 1.0
Nodes (1): Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r

### Community 204 - "Community 204"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr

### Community 205 - "Community 205"
Cohesion: 1.0
Nodes (1): Load cached market_intelligence.macro. Refreshes once per minute.

### Community 206 - "Community 206"
Cohesion: 1.0
Nodes (1): Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur

### Community 207 - "Community 207"
Cohesion: 1.0
Nodes (1): Sum live signed deltas across legs. Returns None if any leg can't be quoted.

### Community 208 - "Community 208"
Cohesion: 1.0
Nodes (1): Returns (should_close, exit_reason).      For Beta trades (exit_rules present):

### Community 209 - "Community 209"
Cohesion: 1.0
Nodes (1): Return the active broker singleton.      Reads BROKER_TYPE env var (alpaca|ibkr)

### Community 210 - "Community 210"
Cohesion: 1.0
Nodes (1): Beta-specific exit rules sourced from trade['exit_rules']. Returns     (should_c

### Community 211 - "Community 211"
Cohesion: 1.0
Nodes (1): Post a message to Discord via bot token. Falls back to webhook if set.

### Community 212 - "Community 212"
Cohesion: 1.0
Nodes (1): Post a trade execution alert to #alerts channel.

### Community 213 - "Community 213"
Cohesion: 1.0
Nodes (1): OCC format: SYMBOL + YYMMDD + C/P + 8-digit strike (strike*1000, zero-padded)

### Community 214 - "Community 214"
Cohesion: 1.0
Nodes (1): Convert '7DTE', '0DTE' etc to YYYY-MM-DD. Never returns today.

### Community 215 - "Community 215"
Cohesion: 1.0
Nodes (1): Query Alpaca options contracts for available strikes on a given expiry.     Retu

### Community 216 - "Community 216"
Cohesion: 1.0
Nodes (1): Find the strike closest to target from available list.

### Community 217 - "Community 217"
Cohesion: 1.0
Nodes (1): Find two real strikes for a spread from Alpaca's chain.     Returns (short_strik

### Community 218 - "Community 218"
Cohesion: 1.0
Nodes (1): Place a multi-leg options order as a single mleg order.     This avoids the 'unc

### Community 219 - "Community 219"
Cohesion: 1.0
Nodes (1): Bear call spread: SELL lower call + BUY higher call.

### Community 220 - "Community 220"
Cohesion: 1.0
Nodes (1): Iron condor: bull put spread + bear call spread as single 4-leg mleg order.

### Community 221 - "Community 221"
Cohesion: 1.0
Nodes (1): Fallback for any other spread — detect put/call from legs and build mleg.

### Community 222 - "Community 222"
Cohesion: 1.0
Nodes (1): Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n

### Community 223 - "Community 223"
Cohesion: 1.0
Nodes (1): Route to correct executor based on strategy. Returns fill or None.

### Community 224 - "Community 224"
Cohesion: 1.0
Nodes (1): Return lines from `path` whose timestamp (if parseable) is >= cutoff.     If a l

### Community 225 - "Community 225"
Cohesion: 1.0
Nodes (1): Stable signature from the stable parts of an error line.

### Community 226 - "Community 226"
Cohesion: 1.0
Nodes (1): Short id derived from signature — stable across runs.

### Community 227 - "Community 227"
Cohesion: 1.0
Nodes (1): Case-insensitive substring check of each catalog pattern against the     raw sig

### Community 228 - "Community 228"
Cohesion: 1.0
Nodes (1): One-line trade summary.

### Community 229 - "Community 229"
Cohesion: 1.0
Nodes (1): Build OCC option symbol from journal leg fields.     e.g. XOM 2026-06-18 call 15

### Community 230 - "Community 230"
Cohesion: 1.0
Nodes (1): Merge updates into matching journal entries, rewrite atomically.     updates: {t

### Community 231 - "Community 231"
Cohesion: 1.0
Nodes (1): GET /v2/positions → {occ_symbol: position_dict} or None on error.     None means

### Community 232 - "Community 232"
Cohesion: 1.0
Nodes (1): Sum unrealized_pl across all legs present in Alpaca.     Returns (total_pnl, leg

### Community 233 - "Community 233"
Cohesion: 1.0
Nodes (1): Build reversed legs for a closing mleg order.     Skips legs with no active Alpa

### Community 234 - "Community 234"
Cohesion: 1.0
Nodes (1): Place market mleg close order. Returns order dict on success, None on failure.

### Community 235 - "Community 235"
Cohesion: 1.0
Nodes (1): Check all four exit rules in priority order.     Returns (should_close: bool, ex

### Community 236 - "Community 236"
Cohesion: 1.0
Nodes (1): QuantAI System Overview

### Community 237 - "Community 237"
Cohesion: 1.0
Nodes (1): Roadmap Current State (Paper Trading Live)

### Community 238 - "Community 238"
Cohesion: 1.0
Nodes (1): Setup Guide Overview

### Community 239 - "Community 239"
Cohesion: 1.0
Nodes (1): Hetzner VPS Setup

### Community 240 - "Community 240"
Cohesion: 1.0
Nodes (1): Git Rules (local main is source of truth)

### Community 241 - "Community 241"
Cohesion: 1.0
Nodes (1): Prevent Accidental Damage Rules

### Community 242 - "Community 242"
Cohesion: 1.0
Nodes (1): Journal Agent Stats Commands

### Community 243 - "Community 243"
Cohesion: 1.0
Nodes (1): Journal Agent Weekly Digest Format

### Community 244 - "Community 244"
Cohesion: 1.0
Nodes (1): Research Agent Soul (intelligence engine)

### Community 245 - "Community 245"
Cohesion: 1.0
Nodes (1): Cron Pipeline

### Community 246 - "Community 246"
Cohesion: 1.0
Nodes (1): aiohttp (orchestrator dep)

### Community 247 - "Community 247"
Cohesion: 1.0
Nodes (1): apscheduler

### Community 248 - "Community 248"
Cohesion: 1.0
Nodes (1): uvicorn

### Community 249 - "Community 249"
Cohesion: 1.0
Nodes (1): pydantic

### Community 250 - "Community 250"
Cohesion: 1.0
Nodes (1): Environment Variables

### Community 251 - "Community 251"
Cohesion: 1.0
Nodes (1): Git Rules (local main is source of truth)

### Community 252 - "Community 252"
Cohesion: 1.0
Nodes (1): Design Principles

### Community 253 - "Community 253"
Cohesion: 1.0
Nodes (1): Iron Condors (Tertiary)

### Community 254 - "Community 254"
Cohesion: 1.0
Nodes (1): Beta Directional Debit Spreads

### Community 255 - "Community 255"
Cohesion: 1.0
Nodes (1): Beta Iron Condors

### Community 256 - "Community 256"
Cohesion: 1.0
Nodes (1): Beta VIX Mean Reversion Spreads

### Community 257 - "Community 257"
Cohesion: 1.0
Nodes (1): Beta Long Straddles (SQUEEZE)

### Community 258 - "Community 258"
Cohesion: 1.0
Nodes (1): Gamma Instrument Universe (27 instruments)

### Community 259 - "Community 259"
Cohesion: 1.0
Nodes (1): Gamma Two-Phase Daily Cycle

### Community 260 - "Community 260"
Cohesion: 1.0
Nodes (1): Regime Classification Errors

### Community 261 - "Community 261"
Cohesion: 1.0
Nodes (1): Pull Full Ticker Universe Principle

### Community 262 - "Community 262"
Cohesion: 1.0
Nodes (1): Strike Snapping Process

### Community 263 - "Community 263"
Cohesion: 1.0
Nodes (1): Slippage Estimation

### Community 264 - "Community 264"
Cohesion: 1.0
Nodes (1): IV Practical Filters Table

### Community 265 - "Community 265"
Cohesion: 1.0
Nodes (1): Beta Plan v1 Phases (13 phases)

### Community 266 - "Community 266"
Cohesion: 1.0
Nodes (1): Beta Plan v2 Reuse vs New Table

### Community 267 - "Community 267"
Cohesion: 1.0
Nodes (1): Auto-Heal Operator Commands

### Community 268 - "Community 268"
Cohesion: 1.0
Nodes (1): Close Order Failed Symptom

## Knowledge Gaps
- **1025 isolated node(s):** `Shared fixtures and assertion helpers for the self-learning test suite.`, `Create a sandboxed QUANTAI_RUNTIME_ROOT and wire all _paths constants.      Yiel`, `Return the trade journal path inside the sandbox.`, `Write two sample trades to the journal and return the path.`, `Hand-written assertion that a diagnosis dict matches the expected schema.` (+1020 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 39`** (7 nodes): `Unit tests for _paths.py — env-var override and constant structure.`, `test_dashboard_state_override()`, `test_defaults_point_under_root()`, `test_env_override_redirects_all_paths()`, `test_journal_ends_with_jsonl()`, `test_learning_tracker_ends_with_json()`, `test_paths.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (7 nodes): `Smoke test: run system_monitor.py against the live VPS.  Verifies the report fil`, `TestLiveHealthReport`, `.test_all_13_checks_present()`, `.test_each_check_has_status()`, `.test_envelope_shape()`, `.test_report_exists()`, `test_sentinel_live_health_report.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (4 nodes): `check()`, `check_keys()`, `section()`, `test_broker.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 52`** (2 nodes): `Central path constants for all self-learning scripts.  All scripts that referenc`, `_paths.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (2 nodes): `Script Inventory`, `system_test.py (43-check health test)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 54`** (2 nodes): `Key File Paths`, `Security Rules (never expose .env, credentials)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (2 nodes): `Research Agent Operating Manual`, `Research Agent Core Beliefs (edge via research)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (2 nodes): `Cron Schedule (UTC)`, `Auto-Heal Schedule (4x/day)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 57`** (2 nodes): `Infra Health Check Procedure`, `Scripts Inventory`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 58`** (2 nodes): `Agent Beta Core Principles`, `Agent Beta — What I Do NOT Do`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 59`** (2 nodes): `Agent Gamma Core Principles`, `Agent Gamma — What I Do NOT Do`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 60`** (2 nodes): `Credit Spreads (Secondary)`, `IV Skew (Put vs Call)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 61`** (2 nodes): `Severity-to-Discord Behavior`, `Logger Sources Ingested (systemd/docker/textlog/syslog/pyapp)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 63`** (1 nodes): `autonomous_execution.py does `os.makedirs("/root/quantai-v2/...")`         at im`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 70`** (1 nodes): `Log a decision. Categories: rule_change, strategy, architecture, bug_fix, config`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 71`** (1 nodes): `# TODO: Phase 2 — trigger Research Agent via orchestrator`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 72`** (1 nodes): `# TODO: Phase 2 — trigger Analysis Agent`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 73`** (1 nodes): `# TODO: Write halt flag to shared state / call orchestrator`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 74`** (1 nodes): `# TODO: Forward to execution agent`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 75`** (1 nodes): `Equity options trade 09:30–16:00 ET on weekdays.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 76`** (1 nodes): `Fetch open positions through the active broker.      Returns {occ_symbol: positi`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 77`** (1 nodes): `Resolve a leg's OCC symbol. Beta journal entries store the broker-     correct O`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 78`** (1 nodes): `Sum unrealized_pl across all legs present in Alpaca.     Returns (total_pnl, leg`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 79`** (1 nodes): `Build reversed legs for a closing mleg order.     Skips legs with no active Alpa`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 80`** (1 nodes): `Place a close order through the active broker. 1 leg → plain order;     2-4 legs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 81`** (1 nodes): `Days to the soonest-expiring leg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 82`** (1 nodes): `Load cached market_intelligence.macro. Refreshes once per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 83`** (1 nodes): `Read current Beta regime from dashboard state. Cached per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 84`** (1 nodes): `Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 85`** (1 nodes): `Sum live signed deltas across legs. Returns None if any leg can't be quoted.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 86`** (1 nodes): `Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 87`** (1 nodes): `Agent Gamma exit checks (Connors RSI pullback).      Triggers (in order):`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (1 nodes): `Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (1 nodes): `Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (1 nodes): `Return True if we have not alerted for *symbol* within the cooldown window.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 91`** (1 nodes): `Compare broker option positions against journal entries.      THREE failure mode`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 92`** (1 nodes): `Return the root crontab as a string, or None if unavailable.      The pipeline c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 93`** (1 nodes): `Verify each expected cron entry is present in the root crontab.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 94`** (1 nodes): `Alert message must tell operator what to check.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 95`** (1 nodes): `Fail-open: corrupt cooldown file → allow alert.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 96`** (1 nodes): `Post a message to Discord via bot token. Falls back to webhook if set.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 97`** (1 nodes): `Post a trade execution alert to #alerts channel.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 98`** (1 nodes): `OCC format: SYMBOL + YYMMDD + C/P + 8-digit strike (strike*1000, zero-padded)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 99`** (1 nodes): `Convert '7DTE', '0DTE' etc to YYYY-MM-DD. Never returns today.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 100`** (1 nodes): `Query the active broker for available strikes on a given expiry.     Returns sor`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 101`** (1 nodes): `Find the strike closest to target from available list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 102`** (1 nodes): `Find two real strikes for a spread from Alpaca's chain.     Returns (short_strik`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 103`** (1 nodes): `Place a multi-leg options order as a single mleg order.     This avoids the 'unc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 104`** (1 nodes): `Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 105`** (1 nodes): `Bear call spread: SELL lower call + BUY higher call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 106`** (1 nodes): `Iron condor: bull put spread + bear call spread as single 4-leg mleg order.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 107`** (1 nodes): `Fallback for any other spread — detect put/call from legs and build mleg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 108`** (1 nodes): `Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 109`** (1 nodes): `Route to correct executor based on strategy. Returns fill or None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 110`** (1 nodes): `Place a close order through the active broker. 1 leg → plain order;     2-4 legs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 111`** (1 nodes): `Days to the soonest-expiring leg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 112`** (1 nodes): `Load cached market_intelligence.macro. Refreshes once per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 113`** (1 nodes): `Read current Beta regime from dashboard state. Cached per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 114`** (1 nodes): `Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 115`** (1 nodes): `Sum live signed deltas across legs. Returns None if any leg can't be quoted.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 116`** (1 nodes): `Return the latest cached daily indicators for `symbol` from     gamma_indicator_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 117`** (1 nodes): `Agent Gamma exit checks (Connors RSI pullback).      Triggers (in order):`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 118`** (1 nodes): `Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 119`** (1 nodes): `Return True if we have not alerted for *symbol* within the cooldown window.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (1 nodes): `Compare broker option positions against journal open entries.      Fires a 🔴 Dis`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 121`** (1 nodes): `Stocks under $30 suitable for Amit's manual collar strategy.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 122`** (1 nodes): `Pattern must match the exact string logged by _broker_ibkr.py.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (1 nodes): `Runbook must include the fix command so it's actionable from Discord.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 124`** (1 nodes): `Auto-action must be 'none' — gateway restart requires human confirmation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 125`** (1 nodes): `Runbook should reference the IBC config fix so it's self-contained.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 126`** (1 nodes): `Runbook must include a way to verify the fix worked.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 127`** (1 nodes): `If everything placed cleanly, delete the pending file. If anything     failed, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (1 nodes): `Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (1 nodes): `Bear call spread: SELL lower call + BUY higher call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 130`** (1 nodes): `Iron condor: bull put spread + bear call spread as single 4-leg mleg order.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (1 nodes): `Fallback for any other spread — detect put/call from legs and build mleg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (1 nodes): `Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (1 nodes): `Route to correct executor based on strategy. Returns fill or None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (1 nodes): `Read persistent probe failure state (survives process restarts).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `Persist probe state; return updated state dict.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (1 nodes): `NYSE equity options hours: 09:30 → 16:00 ET, Mon-Fri.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 137`** (1 nodes): `Decide whether to print the pipeline-beat status line this cron tick.     During`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `Persist the last-print timestamp + status. Failure is non-fatal.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 139`** (1 nodes): `Return age in seconds of an ISO8601 timestamp. 0 if missing/unparseable.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 140`** (1 nodes): `Map RSI(10) reading to a 1-10 conviction score.      Connors threshold is 30. De`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 141`** (1 nodes): `Map Beta strike-selection comfort to a 1-10 conviction score.      Heuristic: ho`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 142`** (1 nodes): `Map debate-chamber judge score (0-100) to a 1-10 conviction score.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 143`** (1 nodes): `If everything placed cleanly, delete the pending file. If anything     failed, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `Merge `fields` into the entry whose id == trade_id. Atomic rewrite.      Returns`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (1 nodes): `Return the entry matching trade_id, or None if absent.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 146`** (1 nodes): `Get current price — tries fast_info first (no 404s), falls back to info.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 147`** (1 nodes): `Get average volume without triggering fundamentals 404s.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (1 nodes): `Pull liquid, optionable tickers from multiple sources.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (1 nodes): `Poor man's covered call/put — diagonal spread.     Sell near-term option, buy fu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (1 nodes): `Iron condors — range-bound, collect from both sides.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (1 nodes): `Stocks under $30 suitable for Amit's manual collar strategy.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `If everything placed cleanly, delete the pending file. If anything     failed, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (1 nodes): `Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 154`** (1 nodes): `Bear call spread: SELL lower call + BUY higher call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (1 nodes): `Iron condor: bull put spread + bear call spread as single 4-leg mleg order.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 156`** (1 nodes): `Fallback for any other spread — detect put/call from legs and build mleg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 157`** (1 nodes): `Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 158`** (1 nodes): `Route to correct executor based on strategy. Returns fill or None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 159`** (1 nodes): `Write UTC timestamp to pipeline beat file so heartbeat_monitor can check livenes`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 160`** (1 nodes): `Read current Beta regime from dashboard state. Cached per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 161`** (1 nodes): `Read current Beta regime from dashboard state. Cached per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 162`** (1 nodes): `Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 163`** (1 nodes): `Sum live signed deltas across legs. Returns None if any leg can't be quoted.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 164`** (1 nodes): `Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 165`** (1 nodes): `Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 166`** (1 nodes): `Bear call spread: SELL lower call + BUY higher call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 167`** (1 nodes): `Iron condor: bull put spread + bear call spread as single 4-leg mleg order.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 168`** (1 nodes): `Fallback for any other spread — detect put/call from legs and build mleg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 169`** (1 nodes): `Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 170`** (1 nodes): `Route to correct executor based on strategy. Returns fill or None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 171`** (1 nodes): `Build reversed legs for a closing mleg order.     Skips legs with no active Alpa`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 172`** (1 nodes): `Place a close order through the active broker. 1 leg → plain order;     2-4 legs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 173`** (1 nodes): `Days to the soonest-expiring leg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (1 nodes): `Load cached market_intelligence.macro. Refreshes once per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (1 nodes): `Read current Beta regime from dashboard state. Cached per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 176`** (1 nodes): `Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 177`** (1 nodes): `Returns (should_close, exit_reason).      For Beta trades (exit_rules present):`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 178`** (1 nodes): `Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 179`** (1 nodes): `Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 180`** (1 nodes): `Build the leg dict in the broker.place_mleg_order shape.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 181`** (1 nodes): `Post a trade execution alert to #alerts channel.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (1 nodes): `OCC format: SYMBOL + YYMMDD + C/P + 8-digit strike (strike*1000, zero-padded)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (1 nodes): `Convert '7DTE', '0DTE' etc to YYYY-MM-DD. Never returns today.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 184`** (1 nodes): `Query the active broker for available strikes on a given expiry.     Returns sor`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (1 nodes): `Find the strike closest to target from available list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (1 nodes): `Find two real strikes for a spread from Alpaca's chain.     Returns (short_strik`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (1 nodes): `Place a multi-leg options order as a single mleg order.     This avoids the 'unc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 188`** (1 nodes): `Bull put spread: SELL higher put + BUY lower put.     Both legs submitted as sin`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (1 nodes): `Bear call spread: SELL lower call + BUY higher call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `Iron condor: bull put spread + bear call spread as single 4-leg mleg order.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (1 nodes): `Route to correct executor based on strategy. Returns fill or None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (1 nodes): `Write UTC timestamp to pipeline beat file so heartbeat_monitor can check livenes`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (1 nodes): `Build OCC option symbol from journal leg fields.     e.g. XOM 2026-06-18 call 15`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (1 nodes): `Equity options trade 09:30–16:00 ET on weekdays.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (1 nodes): `Merge updates into matching journal entries, rewrite atomically.     updates: {t`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `Fetch open positions through the active broker.      Returns {occ_symbol: positi`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `Sum unrealized_pl across all legs present in Alpaca.     Returns (total_pnl, leg`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `Build reversed legs for a closing mleg order.     Skips legs with no active Alpa`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `Place a close order through the active broker. 1 leg → plain order;     2-4 legs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (1 nodes): `Check all four exit rules in priority order.     Returns (should_close: bool, ex`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Load cached market_intelligence.macro. Refreshes once per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Sum live signed deltas across legs. Returns None if any leg can't be quoted.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Beta-specific exit rules from trade['exit_rules'].      Returns (should_close, r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Returns (should_close, exit_reason, close_fraction, partial_flag).      close_fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Load cached market_intelligence.macro. Refreshes once per minute.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Today's SPX open vs yesterday's close, %. Cached per day. Returns None on failur`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `Sum live signed deltas across legs. Returns None if any leg can't be quoted.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `Returns (should_close, exit_reason).      For Beta trades (exit_rules present):`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `Return the active broker singleton.      Reads BROKER_TYPE env var (alpaca|ibkr)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `Beta-specific exit rules sourced from trade['exit_rules']. Returns     (should_c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Post a message to Discord via bot token. Falls back to webhook if set.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Post a trade execution alert to #alerts channel.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `OCC format: SYMBOL + YYMMDD + C/P + 8-digit strike (strike*1000, zero-padded)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `Convert '7DTE', '0DTE' etc to YYYY-MM-DD. Never returns today.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `Query Alpaca options contracts for available strikes on a given expiry.     Retu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `Find the strike closest to target from available list.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `Find two real strikes for a spread from Alpaca's chain.     Returns (short_strik`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `Place a multi-leg options order as a single mleg order.     This avoids the 'unc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `Bear call spread: SELL lower call + BUY higher call.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `Iron condor: bull put spread + bear call spread as single 4-leg mleg order.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Fallback for any other spread — detect put/call from legs and build mleg.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `Diagonal spread (poor man's covered call/put).     BUY far-dated option + SELL n`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `Route to correct executor based on strategy. Returns fill or None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `Return lines from `path` whose timestamp (if parseable) is >= cutoff.     If a l`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `Stable signature from the stable parts of an error line.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `Short id derived from signature — stable across runs.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `Case-insensitive substring check of each catalog pattern against the     raw sig`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `One-line trade summary.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `Build OCC option symbol from journal leg fields.     e.g. XOM 2026-06-18 call 15`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `Merge updates into matching journal entries, rewrite atomically.     updates: {t`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `GET /v2/positions → {occ_symbol: position_dict} or None on error.     None means`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `Sum unrealized_pl across all legs present in Alpaca.     Returns (total_pnl, leg`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `Build reversed legs for a closing mleg order.     Skips legs with no active Alpa`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `Place market mleg close order. Returns order dict on success, None on failure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `Check all four exit rules in priority order.     Returns (should_close: bool, ex`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `QuantAI System Overview`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `Roadmap Current State (Paper Trading Live)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Setup Guide Overview`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `Hetzner VPS Setup`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Git Rules (local main is source of truth)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Prevent Accidental Damage Rules`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `Journal Agent Stats Commands`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `Journal Agent Weekly Digest Format`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (1 nodes): `Research Agent Soul (intelligence engine)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Cron Pipeline`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (1 nodes): `aiohttp (orchestrator dep)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (1 nodes): `apscheduler`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `uvicorn`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 249`** (1 nodes): `pydantic`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `Environment Variables`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `Git Rules (local main is source of truth)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): `Design Principles`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 253`** (1 nodes): `Iron Condors (Tertiary)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (1 nodes): `Beta Directional Debit Spreads`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 255`** (1 nodes): `Beta Iron Condors`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 256`** (1 nodes): `Beta VIX Mean Reversion Spreads`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `Beta Long Straddles (SQUEEZE)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 258`** (1 nodes): `Gamma Instrument Universe (27 instruments)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 259`** (1 nodes): `Gamma Two-Phase Daily Cycle`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 260`** (1 nodes): `Regime Classification Errors`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `Pull Full Ticker Universe Principle`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 262`** (1 nodes): `Strike Snapping Process`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `Slippage Estimation`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 264`** (1 nodes): `IV Practical Filters Table`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (1 nodes): `Beta Plan v1 Phases (13 phases)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `Beta Plan v2 Reuse vs New Table`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 267`** (1 nodes): `Auto-Heal Operator Commands`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `Close Order Failed Symptom`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `BrokerBase` connect `Community 0` to `Community 12`, `Community 20`, `Community 6`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `Client` connect `Community 1` to `Community 26`, `Community 3`, `Community 6`, `Community 14`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `_is_in_restart_window()` connect `Community 6` to `Community 0`, `Community 2`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Are the 94 inferred relationships involving `Client` (e.g. with `TestClawRouteReachable` and `TestLiveDiagnose`) actually correct?**
  _`Client` has 94 INFERRED edges - model-reasoned connections that need verification._
- **Are the 81 inferred relationships involving `BrokerBase` (e.g. with `IBKRBroker` and `_IBKRNoiseFilter`) actually correct?**
  _`BrokerBase` has 81 INFERRED edges - model-reasoned connections that need verification._
- **Are the 35 inferred relationships involving `check_risk()` (e.g. with `.test_allowed_with_no_open_positions()` and `.test_allowed_with_two_open_positions()`) actually correct?**
  _`check_risk()` has 35 INFERRED edges - model-reasoned connections that need verification._
- **Are the 17 inferred relationships involving `IBKRBroker` (e.g. with `_OptionSpec` and `BrokerBase`) actually correct?**
  _`IBKRBroker` has 17 INFERRED edges - model-reasoned connections that need verification._