...
- Every change must include reasoning"""
    ...
    if GITHUB_TOKEN:
        await create_improvement_pr(fix_data, today, score)
    else:
        # No GitHub token — just post to Discord
        await post_improvement_to_discord(fix_data, today, score)


async def create_improvement_pr(fix_data, today, score):
...
Merge only after reviewing the changes. All changes will run on paper for 2 weeks before live promotion.
"""
